...
<div class="max-w-container-max mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-12">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        ...
        <div class="flex gap-3">
            @if($unreadCount > 0)
                <form action="{{ route('reviewer.notifikasi.mark-all-read') }}" method="POST">
                    @csrf
            ...
            @endif
            <form action="{{ route('reviewer.notifikasi.clear-read') }}" method="POST" class="inline">
                @csrf
        ...
        @if($notifications->count() > 0)
            <div class="divide-y divide-outline-variant">
                @foreach($notifications as $notification)
                    ...
                    <a href="{{ route('reviewer.notifikasi.redirect', $notification->id) }}" 
...
<script>
    function markAsRead(id) {
        fetch(`/reviewer/notifikasi/mark-read/${id}`, {
            method: 'POST',
            headers: {
